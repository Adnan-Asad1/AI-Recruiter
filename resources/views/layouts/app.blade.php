...
<body class="bg-gradient-to-br from-blue-100 to-blue-50 text-gray-800 font-sans">
  ...
  <header class="fixed top-0 left-0 w-full border-b bg-white z-20">
    <div class="px-4 md:px-10 flex h-16 items-center justify-between">

      <div class="flex items-center gap-3">
        {{-- 🔸 Hamburger Button (mobile only, auth only) --}}
        @auth
          <button id="sidebar-toggle"
            class="md:hidden flex items-center justify-center w-9 h-9 rounded-md text-gray-600 hover:bg-gray-100 transition"
        ...
        <div class="flex items-center gap-2">
          <img src="{{ asset('logos.png') }}" alt="Smart Hire Logo" width="140" class="h-auto object-contain" />
        </div>
      </div>
      ...
      <div class="flex items-center gap-2 md:gap-4">
        @guest
          <a href="/login">
            <button
              class="bg-[#0377FC] text-white hover:bg-indigo-700 h-9 px-3 md:px-4 py-2 rounded-md shadow text-sm font-medium">
              Login
            </button>
          </a>
          <a href="/register">
  ...
  @auth
    <div id="sidebar-backdrop" class="fixed inset-0 bg-black/50 z-30 hidden md:hidden" onclick="closeSidebar()"></div>
  @endauth
  ...
  <div class="pt-16 flex">
    ...
    @auth
      <aside id="sidebar" class="fixed top-16 left-0 w-64 h-[calc(100vh-4rem)] bg-gray-50 border-r shadow-2xl p-4 z-40 pt-12
        -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out">

        {{-- Close button (mobile only) --}}
        <button onclick="closeSidebar()"
          class="md:hidden absolute top-3 right-3 text-gray-500 hover:text-red-500 text-xl font-bold">✕</button>

        {{-- 🔵 Create Interview Button --}}
        <div class="mb-8">
          <a href="/interview/create" onclick="closeSidebar()"
        ...
        <ul class="space-y-3">
          ...
          @foreach($links as $link)
            ...
            @endphp
            <li>
              <a href="{{ url($link['url']) }}" onclick="closeSidebar()"
    ...
    {{-- ✅ Main Page Content --}}
    <main class="flex-1 bg-white @auth md:pl-64 @endauth min-h-[calc(100vh-4rem)] w-full">
      {{ $slot }}
    </main>
