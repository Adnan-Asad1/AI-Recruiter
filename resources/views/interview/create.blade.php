<x-app-layout>
  ...
  <div class="flex bg-gradient-to-br from-blue-100 to-blue-50 pt-6 px-4 md:px-8 pb-24 min-h-[91vh]">
      ...
      <main>
        ...
        <div class="flex justify-center mt-6">
          <div class="relative w-2/3 h-3 bg-gray-200 rounded-full overflow-hidden shadow-inner">
            <div
              class="absolute top-0 left-0 h-3 bg-gradient-to-r from-indigo-500 to-purple-500 transition-all duration-500 rounded-full"
              style="width: 30%;"></div>
          </div>
        </div>

        <!-- Form Card -->
        <div class="max-w-3xl mx-auto bg-white p-6 md:p-10 rounded-2xl shadow-xl mt-10 border border-gray-100">
          <h2 class="text-2xl font-bold text-indigo-600 mb-8 flex items-center gap-2">
            📝 Create Interview Details
          </h2>

          <form action="{{ route('generate.questions') }}" method="POST" class="space-y-8"
            ...
            <div>
              ...
              <textarea id="job_description" name="job_description" rows="6"
                placeholder="Write a detailed description about the role..."
                class="w-full min-h-[140px] border border-gray-200 rounded-xl px-4 py-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200"
                required></textarea>
            </div>
            ...
            <div class="flex flex-col md:flex-row gap-6">
              ...
              <div class="w-full md:w-1/2">
                ...
                <input type="number" id="num_questions" name="num_questions" placeholder="e.g. 5" min="1" max="25"
                  required
                  class="w-full border border-gray-200 rounded-xl px-4 py-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition duration-200">
            ...
            <div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                @foreach (['Technical', 'Behavioral', 'Experience', 'Problem Solving', 'Leadership'] as $type)
                  <label
                    class="flex items-center space-x-3 p-3 border border-gray-200 rounded-xl shadow-sm hover:bg-indigo-50 cursor-pointer transition">
                    <input type="checkbox" name="interview_type[]" value="{{ $type }}"
